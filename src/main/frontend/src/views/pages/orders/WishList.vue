<template>
  <div id="ecommerce-wishlist-demo">
        <div class="items-grid-view vx-row match-height" v-if="wishListitems.length" appear>
            <div class="vx-col lg:w-1/6 md:w-1/3 sm:w-1/2 w-full" v-for="item in wishListitems" :key="item.inventoryId">
                    ...
                    <template slot="action-buttons">
                        <div class="flex flex-wrap">
                            ...
                            <div
                                ...
                                <span class="text-sm font-semibold ml-2" v-if="isInCart(item.inventoryId)">VIEW IN CART</span>
        ...
        <!-- IF NO ITEMS IN CART -->
        <vx-card :title="$t('youDontHaveWishList')" v-else>
            <vs-button @click="$router.push('/pages/orders/shop')">{{$t('browseShop')}}</vs-button>
        </vx-card>
  </div>
</template>
<script>

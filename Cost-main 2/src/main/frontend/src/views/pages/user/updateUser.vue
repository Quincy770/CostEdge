      ...
      <vx-card id="card-id" :title="$t('updateUser')">
        <vx-card id="internal-card" class="mt-10" title no-shadow card-border>
          <div class="vx-row mt-0">
            ...
            <div class="vx-col sm:w-1/4 w-full mb-6">
              <vs-input class="w-full" :label="$t('Middle Name')" v-model="lastName" name="lastName"
                />
              <span class="text-danger">{{ errors.first('lastName') }}</span>
            </div>
            <div class="vx-col sm:w-1/4 w-full mb-6">
              <vs-input class="w-full" :label="$t('Last Name')" v-model="grandFatherName" name="grandFatherName"
                v-validate="'required|alpha'" data-vv-validate-on="blur" />
          ...
          <div class="vx-row">
            <div class="vx-col sm:w-1/4 w-full mb-2">
              <vs-input icon-pack="feather" icon="icon-country-code" icon-no-border class="w-full"
                ...
                data-vv-validate-on="blur" @keyup="space" />
            ...
            <div class="vx-col w-1/4 mb-6 mr-4 ml-1">
              ...
              <v-select v-model="role" v-validate="'required'" class="w-full"
                :options="roles" v-validate:role="'required'"
          ...
          <div class="vx-row">
            <div class="vx-col w-3/4">
              <vs-button style="float:right;" @click="updateUser" class=" mb-2">{{ $t('Update') }}</vs-button>
            </div>
          </div>
          <p></p>
        </vx-card>
        ...
        if (result && this.mobilePhone != "") {
          ...
          this.$http
            .put("/accounts/" + this.id, {
              ...
              mobilePhone: "+44" + this.mobilePhone.substr(1),
              email: this.email,
              userType: this.role,
